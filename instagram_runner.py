...
COMMAND_NAME = CONFIG.get("INSTAGRAM_COMMAND_NAME", "Instagram")
...
BOT_VERSION = "2025-12-06_instagram_v1"
...
def process_cluster(service, settings, cluster_name, cluster_data, with_gpt=True):
    ...
    for p in posts:
        ...
        if p.get("hashtags"):
            try:
                hashtags_val = json.dumps(p.get("hashtags"), ensure_ascii=False)
            except Exception:
                hashtags_val = ""

        new_row = [
            p.get("url", ""),
            p.get("play_count") or p.get("playcount") or p.get("like_count") or "",
            hashtags_val,
            p.get("profile_url") or p.get("owner_profile") or "",
            followers_val,
            p.get("profile_biography") or p.get("description") or p.get("owner_biography") or "",
            batch_label,
            "",
        ]
...
def _run_over_active_clusters(service, settings, with_gpt=True, run_label="run_insta"):
    clusters = load_instagram_clusters(service)
    ...
    if not active_clusters:
        print("Нет активных Instagram-кластеров.")
        write_log(service, "no_active_clusters", "Instagram", run_label)
        return
    ...
    for cluster_name, cluster_data in active_clusters:
        try:
            ...
            update_setting(service, "last_cluster_name_instagram", cluster_name)
        except Exception as e:
            print(
...
def run_once():
    ...
    write_log(service, "run_start", "Instagram", f"version={BOT_VERSION}")
    print(f"[RUN] Старт Instagram-кластеров. Версия: {BOT_VERSION}")

    settings = load_settings(service)
    _run_over_active_clusters(service, settings, with_gpt=True, run_label="run_insta")


def run_scrape_only():
    service = get_sheets_service()
    write_log(service, "scrape_start", "Instagram", f"version={BOT_VERSION}")
    print(f"[SCRAPE_ONLY] Instagram. Версия: {BOT_VERSION}")

    settings = load_settings(service)
    _run_over_active_clusters(service, settings, with_gpt=False, run_label="scrape_insta")
...
def run_gpt_only(overwrite=False):
    ...
    rows, processed = apply_gpt_labels(
        service,
        cluster_name="GPT_ONLY_INSTAGRAM",
        header=header,
        rows=rows,
